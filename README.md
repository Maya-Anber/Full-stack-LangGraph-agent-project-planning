...
erDiagram
    ...
    CONVERSATION ||--o{ ESCALATION : escalates_to
...
| ---------------------------------------------------------------------- | ----------------------------------------------------------------------------------------- |
| AI agent: customer service**and** sales                          | `app/agent/nodes.py` -- two branches behind one router                                  |
| RAG used when answering                                                | `app/rag/` (`vector_store.py`, `retriever.py`), driven by the `retrieve` node     |
| RAG management: add / update / delete, and the agent uses updated data | `/admin/knowledge` CRUD → `app/rag/ingest.py` → the index is rebuilt on every write |
| LangGraph with a meaningful workflow                                   | `app/agent/graph.py` -- classify → retrieve → branch → tool loop → finalize         |
| Business action through tool/function calling                          | `app/agent/tools.py` -- `add_to_cart` and `place_order` create real rows            |
| Flask dashboard (business data + RAG data)                             | `app/routes/dashboard.py` + `app/templates/dashboard/`                                |
| Database with an ORM                                                   | SQLite + SQLAlchemy -- see**Database structure** above                              |
| Bonus: Meta Messenger                                                  | `app/routes/webhook.py` -- the same graph on another channel                            |

## Submission demo checklist

1. Start the app with `python run.py` and open `/` for the customer chat.
2. Ask a policy question and show that the response is grounded in the seeded
   knowledge base.
3. Ask for a product, add it to a cart, and place an order with an email and
   shipping address. Confirm the order and stock change in `/admin/orders`.
4. Edit a knowledge item at `/admin/knowledge`, then ask the related question
   again to demonstrate live retrieval updates.
5. Run `python -m pytest` before submitting. Live Gemini/Groq responses require
   the selected provider key; the automated suite uses a deterministic model
   double and TF-IDF.
...
Things I know are not production-ready, roughly in priority order:

* **No live LLM call has been made yet.** Everything above is verified with a
  mocked model, so routing, tool-calling, persistence and rendering are proven,
  but the exact wording a real Gemini/Groq model produces is not. First task
  with an API key: run the four example conversations and skim
  `Conversation` transcripts for tone/accuracy.
* **MiniLM is local and CPU-bound.** The first startup downloads the model and
  rebuilds the in-memory index; use `RAG_EMBEDDING_BACKEND=tfidf` for a fully
  deterministic no-download test environment.
* **The dashboard has no authentication** and no CSRF protection. It is a demo
  surface: keep it local or put real auth in front of it.
* **The RAG index is per-process and in memory.** With multiple workers each
  process rebuilds its own copy on startup and after its own writes, so a write
  in one worker won't be visible to another. A shared vector DB (or rebuilding on
  a schedule) fixes it.
* **No rate limiting or cost controls** on `/api/chat` -- every message costs an
  LLM call. Add per-IP/session limits before exposing it.
* **An email identifies a customer.** There's no login, so anyone who types
  someone else's address is treated as that customer. Real auth would key carts
  and orders off a session or account instead.
* **No payment step.** `place_order` moves the cart straight to `confirmed`; a
  payment provider would slot in before the status change, after which
  `confirmed` becomes the post-payment state.
* **Messenger is a demo integration.** `X-Hub-Signature` is not verified on
  inbound webhooks, replies are sent synchronously inside the request, and each
  sender gets a placeholder email as described above.
* **A failed turn leaves a user message in the transcript.** If the agent errors
  out (missing key, step limit), the customer's message is already persisted
  without a reply. Deliberate for auditability, but worth tidying if you show
  transcripts to customers.
* **`db.create_all()` instead of migrations.** Fine for SQLite and a demo,
  wrong the moment the schema changes on a real deployment -- add Alembic then.
* **No CI, Dockerfile, or deployment config.** `python run.py` uses Flask's dev
  server; a real deployment wants gunicorn/uwsgi behind a proxy.

Smaller polish items: the classifier could be skipped for obvious greetings,
`MAX_HISTORY_MESSAGES` is a flat turn count rather than a token budget, and the
dashboard's product form doesn't validate that the SKU is unique before hitting
